...
export default function LoginPage() {
  ...
  return <main className="grid min-h-screen lg:grid-cols-2">
    ...
    <aside className="hidden border-l bg-foreground text-background lg:flex lg:flex-col lg:justify-between lg:p-14"><div className="text-sm font-medium">Everything you’re building, one clear view.</div><div><h2 className="max-w-xl text-5xl font-semibold leading-[1.05] tracking-[-.05em]">Your businesses.<br />Your progress.<br />Your personal system.</h2><ul className="mt-10 space-y-3 text-sm opacity-70">{["See what changed across your connected areas","Find the right account quickly","Turn important updates into clear actions"].map((item) => <li key={item} className="flex items-center gap-2"><Check className="size-4" />{item}</li>)}</ul></div><p className="text-xs opacity-50">Nexarch · Your Personal Operating System</p></aside>
  </main>;
}
