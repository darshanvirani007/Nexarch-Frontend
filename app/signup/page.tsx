...
export default function SignUpPage() {
  ...
  return <main className="grid min-h-screen lg:grid-cols-2">
    ...
    <aside className="hidden border-l bg-foreground text-background lg:flex lg:flex-col lg:justify-between lg:p-14"><div className="text-sm font-medium">Everything you’re building, one clear view.</div><div><h2 className="max-w-xl text-5xl font-semibold leading-[1.05] tracking-[-.05em]">One account.<br />One clear view.<br />Your personal system.</h2><ul className="mt-10 space-y-3 text-sm opacity-70">{["Keep your businesses and accounts together","Track your learning, tasks and goals","See what needs your attention"].map((item) => <li key={item} className="flex items-center gap-2"><Check className="size-4" />{item}</li>)}</ul></div><p className="text-xs opacity-50">Nexarch · Your Personal Operating System</p></aside>
  </main>;
}
